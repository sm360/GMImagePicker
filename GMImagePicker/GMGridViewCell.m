...
- (id)initWithFrame:(CGRect)frame
{
	if (self = [super initWithFrame:frame]) {
		...
		_cloudButton.adjustsImageWhenHighlighted = NO;
		[_cloudButton setImage:nil forState:UIControlStateNormal];
		_cloudButton.translatesAutoresizingMaskIntoConstraints = NO;
		_cloudButton.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
		UIImage * _Nullable cloudImage = [UIImage imageNamed:@"GMCloudCross" inBundle:[NSBundle bundleForClass:GMGridViewCell.class] compatibleWithTraitCollection:nil];
		UIImage * _Nullable templateImage = [cloudImage imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
		[_cloudButton setImage:templateImage forState:UIControlStateSelected];
		_cloudButton.tintColor = [UIColor colorWithRed:255/255.0 green:75/255.0 blue:75/255.0 alpha:1];
		_cloudButton.hidden = NO;
		_cloudButton.userInteractionEnabled = NO;
		[self addSubview:_cloudButton];
